...
void UpdateCustomizeScreen(void) {
    ...
    if (GameMenuBusy == 0) {
        ...
        if ((RunTimedDrawScript(g_UiChromeScript, &g_UiScriptProgress, 1) != 0) && (g_UiScriptProgress2 <= 0)) {
            ...
            if (g_PadPressed & PAD_UP) {
                PlaySoundCue(1);
                g_RankingOption = g_RankingOption > 0
                                      ? g_RankingOption - 1
                                      : lastOption;
            }
            if (g_PadPressed & PAD_DOWN) {
                PlaySoundCue(1);
                g_RankingOption = g_RankingOption < lastOption
                                      ? g_RankingOption + 1
                                      : 0;
            }
    ...
    if (GameMenuBusy < 0) {
        if (GameMenuBusy == CUSTOMIZE_TIRE_DIALOG) {
        ...
        } else if (GameMenuBusy == CUSTOMIZE_TRANSMISSION_DIALOG) {
            if (RunTimedDrawScript(g_CustomizePopupScript, &g_UiScriptProgress2, 1) != 0) {
                if (g_PadPressed & PAD_CONFIRM) {
                    PlaySoundCue(2);
        ...
        } else if (GameMenuBusy ==
                   CUSTOMIZE_TRANSMISSION_UNAVAILABLE_CLOSING) {
            RunTimedDrawScript(g_CustomizePopupScript, &g_UiScriptProgress2, -1);
            RunTimedDrawScript(g_UiChromeScript2, &g_UiScriptProgress2, 0);
            if (g_UiScriptProgress2 <= 0) {
                GameMenuBusy = 0;
            }
        } else if (GameMenuBusy == CUSTOMIZE_TIRE_CONFIRMING) {
        ...
        } else if (GameMenuBusy == CUSTOMIZE_TRANSMISSION_CONFIRMING) {
            if (g_MenuConfirmTimer <= 0) {
                RunTimedDrawScript(g_CustomizePopupScript, &g_UiScriptProgress2, -1);
                if (g_UiScriptProgress2 <= 0) {
                    GameMenuBusy = 0;
