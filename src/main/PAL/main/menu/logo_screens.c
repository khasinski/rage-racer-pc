...
static s32 WrapMenuOption(s32 option, s32 step, s32 count) {
    option += step;
    if (option < 0) {
        return count - 1;
    }
    return option >= count ? 0 : option;
}
...
static void UpdateLogoSampleIdle(void) {
    ...
    if (g_PadPressed & PAD_UP) {
        PlaySoundCue(1);
        g_LogoSampleCursor = WrapMenuOption(g_LogoSampleCursor, -1, 3);
    }
    if (g_PadPressed & PAD_DOWN) {
        PlaySoundCue(1);
        g_LogoSampleCursor = WrapMenuOption(g_LogoSampleCursor, 1, 3);
...
static void UpdateLogoSamplePicker(s32 *selection) {
    ...
    if (g_PadPressed & PAD_LEFT) {
        PlaySoundCue(1);
        *selection = WrapMenuOption(*selection, -1, LOGO_SAMPLE_COUNT);
    }
