...
static int ProjectQuad(
    ...
    }
    {
        ...
        if (terrainQuad &&
            ((!g_RenderState.pass.orderingFlag && clip0 <= 0 && clip1 < 0) ||
             (g_RenderState.pass.orderingFlag && clip0 >= 0 && clip1 > 0))) {
            if (terrainQuad == 2)
        ...
        *depth = ((int)otz >> g_RenderState.pass.otShift);
        if ((!terrainQuad && !ModelFaceVisible(g_RenderState.pass.orderingFlag, clip0)) ||
            (terrainQuad &&
             ((!g_RenderState.pass.orderingFlag && clip0 <= 0 && clip1 >= 0) ||
              (g_RenderState.pass.orderingFlag && clip0 >= 0 && clip1 <= 0)))) {
...
static int ProjectCourseFace(
    ...
    g_RageProjectionReject = 0;
    if ((!g_RenderState.pass.orderingFlag && clip <= 0) || (g_RenderState.pass.orderingFlag && clip >= 0)) {
        g_RageProjectionReject = 2;
        return 0;
    }
    if (ScreenQuadOutsideBounds(
...
static int CourseScreenQuadVisible(const int sxy[4]) {
    ...
    if (!CourseQuadVisible(g_RenderState.pass.orderingFlag, clip0, clip1))
        return 0;
...
static uint8_t *EmitTerrainFt4(
    uint8_t *cursor, GameOrderingTableEntry *ot, int depth, int fog, int dispatch,
    ...
    if (!PrimitiveSpaceAvailable(cursor, packetSize)) return NULL;
    ...
    StoreSxy(&poly->x3,&poly->y3,sxy[3]);
    cursor += sizeof(*poly);
    if (dispatch < 2) {
...
static uint8_t *EmitTerrainSubdivisionLines(
    uint8_t *cursor, GameOrderingTableEntry *ot, int depth, const int sxy[4],
    uint32_t command) {
    LINE_F3 *first;
    LINE_F3 *second;
    if (!PrimitiveSpaceAvailable(cursor, sizeof(LINE_F3) * 2))
        return NULL;
...
static void RageSubmitModelFaces(
    ...
    for (i = 0; i < count; i++, faces += strides[type]) {
        ...
        if (!ProjectModelFace(faces, vertices, sxy, &depth, NULL)) {
        ...
        if (type == RAGE_MODEL_F4) {
            if (!PrimitiveSpaceAvailable(cursor, sizeof(POLY_F4))) break;
            ...
            cursor += sizeof(*poly);
            {
                ...
                CaptureFace3D(&capture);
            }
        } else if (type == RAGE_MODEL_FT4) {
            if (!PrimitiveSpaceAvailable(cursor, sizeof(POLY_FT4))) break;
            POLY_FT4 *poly = (POLY_FT4 *)cursor;
            SetPolyFT4(poly);
            SetShadeTex(poly, 1);
            poly->r0 = poly->g0 = poly->b0 = 0x80;
            CopyFt4UvWithMode(poly, faces + 8,
                                  (uint32_t)g_RenderState.geometry.envMode4);
            StoreSxy(&poly->x0, &poly->y0, sxy[0]);
            StoreSxy(&poly->x1, &poly->y1, sxy[1]);
            StoreSxy(&poly->x2, &poly->y2, sxy[2]);
            StoreSxy(&poly->x3, &poly->y3, sxy[3]);
            ...
            if (!PrimitiveSpaceAvailable(cursor, sizeof(POLY_G4))) break;
            ...
            StoreSxy(&poly->x0,&poly->y0,sxy[0]); StoreSxy(&poly->x1,&poly->y1,sxy[1]);
            StoreSxy(&poly->x2,&poly->y2,sxy[2]); StoreSxy(&poly->x3,&poly->y3,sxy[3]);
            ...
            CVECTOR base = {g_RenderState.geometry.gt4Color[0], g_RenderState.geometry.gt4Color[1], g_RenderState.geometry.gt4Color[2],
                            g_RenderState.geometry.gt4Color[3]};
            CVECTOR colors[4];
            SetPolyGT4(poly);
...
void SubmitModel(void *ctx, int index) {
    ...
    uint32_t opcode;
    const void *const *models = g_RenderState.geometry.modelModels;
    (void)ctx;
    ...
    while ((opcode = RageReadU32(stream)) != 0) {
        ...
        RageSubmitModelFaces(type, count, stream,
                             (const SVECTOR *)g_RenderState.geometry.modelTable1,
                             (const SVECTOR *)g_RenderState.geometry.modelNormals);
...
static void RageSubmitCourseModel(int index, int fogged) {
    const NativeCourseModel *models = g_RenderState.geometry.courseBank;
    const uint8_t *stream;
    uint8_t *cursor = RENDER_PRIM_CURSOR_AS(uint8_t);
        ...
        for (i = 0; i < count; i++, stream += stride) {
            ...
            if (!projected) {
                ...
                if (g_RageProjectionReject == 3 &&
                    ModernDepthLimit() > 0 && depth > 0 &&
                    depth < ModernDepthLimit()) {
                    extendedDepth = 1;
                } else {
                    continue;
                }
            ...
            depth += bias;
            {
                ...
                if (type <= 1) CaptureFace3D(&capture);
            }
            ...
            if (type == 0) {
                ...
                int uLevel = stream[26] -
                    (rawDepth >> g_RenderState.pass.faceOtShift);
                 ...
                 * can emit instead. */
                uLevel = ClampSubdivisionLevel(uLevel);
                vLevel = ClampSubdivisionLevel(vLevel);
                uSteps = 1 << uLevel;
                vSteps = 1 << vLevel;
                {
                    RageCaptureFaceInput capture = {0};
                ...
                for (sy = 0; sy < vSteps; sy++) {
                    for (sx = 0; sx < uSteps; sx++) {
                        ...
                        uint8_t subUv[8];
                        uint8_t *next;
                        subSxy[0] = BilerpSxy(sxy,sx,sy,uSteps,vSteps);
...
void SubmitTerrainCells(void *ctx, const VisibleTerrainCell *cells, int count) {
    const void *const *cellTable = g_RenderState.geometry.cellTable;
    const SVECTOR *vertices = g_RenderState.geometry.cellFaces;
    uint8_t *cursor = RENDER_PRIM_CURSOR_AS(uint8_t);
    ...
    if (g_RenderState.pass.orderingFlag) {
        (&g_RenderState.geometry.matrix)->m[0][0] = -(&g_RenderState.geometry.matrix)->m[0][0];
        (&g_RenderState.geometry.matrix)->m[0][1] = -(&g_RenderState.geometry.matrix)->m[0][1];
        (&g_RenderState.geometry.matrix)->m[0][2] = -(&g_RenderState.geometry.matrix)->m[0][2];
    ...
    for (cell = 0; cell < count; cell++, cells++) {
        ...
        translation.vx = g_RenderState.pass.orderingFlag ? -cells->x : cells->x;
        ...
        while ((opcode = RageReadU32(stream)) != 0) {
            ...
            dispatch = mode < 2 ? mode * 2 + (g_RenderState.geometry.envMode4 != 0)
                                : mode - 2;
            ...
            for (faceIndex = 0; faceIndex < faceCount;
                ...
                if (farCell && (stream[20] & 2) != 0) {
                    ...
                    if (ModernDepthLimit() <= 0) continue;
                    extendedDepth = 1;
                }
                {
                ...
                uLevel = stream[22] - (rawDepth >> g_RenderState.pass.faceOtShift);
                ...
                if (uSteps == 1 && vSteps == 1) {
                    uint8_t *next;
                    depth += bias;
                    next = EmitTerrainFt4(cursor, ot, depth, fog, dispatch,
                    ...
                    for (sy = 0; sy < uSteps; sy++) {
                        for (sx = 0; sx < vSteps; sx++) {
                            ...
                            uint8_t uv[8];
                            uint8_t *next;
                            child[0] = BilerpVertex(
                                v0, v1, v2, v3, sy, sx, uSteps, vSteps);
                            child[1] = BilerpVertex(
                                v0, v1, v2, v3, sy + 1, sx,
                                uSteps, vSteps);
                            child[2] = BilerpVertex(
                                v0, v1, v2, v3, sy, sx + 1,
                                uSteps, vSteps);
                            child[3] = BilerpVertex(
                                v0, v1, v2, v3, sy + 1, sx + 1,
                                uSteps, vSteps);
                            {
                                int childVisible = ProjectQuad(
