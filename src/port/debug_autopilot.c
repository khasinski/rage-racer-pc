...
static int Drive(PlayerCarRuntime *player) {
    ...
    if (!s_placed) {
        ...
        s_placed = 1;
        fprintf(stderr, "rage-port: autopilot start point=%d course=%d series=%d hz=%d direction=%d\n",
                s_route.point, g_CourseIndex, g_RaceSeries, TimingBaseHz() / 2,
                s_direction ? s_direction : (g_RaceSeries ? 1 : -1));
    }
    oldLaps = s_route.laps;
    if (!DebugRouteAdvance(&s_route, g_TrackPointCount,
                           s_direction ? s_direction : (g_RaceSeries ? 1 : -1),
                           s_speed, TimingBaseHz() / 2,
    ...
    if (s_route.laps != oldLaps) {
        fprintf(stderr, "rage-port: autopilot lap=%d target=%d point=%d pos=%d,%d section=%d distance_units=%llu\n",
                s_laps, s_target, car->trackPointIndex, car->x, car->z, car->trackSection,
                (unsigned long long)s_route.distance);
...
void DebugAutopilotBeforeScene(void) {
    if (!s_initialized) {
        ...
        s_direction = RuntimeConfigInt("autopilot.direction", 0, -1, 1);
