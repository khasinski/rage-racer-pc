...
int PortShouldExit(int frame_number) {
    ...
    int index;
    const char *toggleRendererFrame;
    const char *restartRaceFrames;
    ...
    if (RuntimeConfigEnabled("hooks.retire") && g_SceneId == 12) {
        if (retireStep == 0 && g_RacePhase == 2 && g_PauseDebounce <= 0) {
            ...
            retireStep = 1;
            retireWait = 0;
        } else if (retireStep == 1 && g_RacePaused && ++retireWait >= 10) {
        ...
        if (length <= 0 || (size_t)length >= sizeof(path) ||
            ...
            if (g_SmokeCaptureManifest != NULL) {
                ...
                fprintf(g_SmokeCaptureManifest,
                        ...
                        (&g_RageScratchpadState)->matrix.m[0][0], (&g_RageScratchpadState)->matrix.m[0][1],
                        (&g_RageScratchpadState)->matrix.m[0][2], (&g_RageScratchpadState)->matrix.m[1][0],
                        (&g_RageScratchpadState)->matrix.m[1][1], (&g_RageScratchpadState)->matrix.m[1][2],
                        (&g_RageScratchpadState)->matrix.m[2][0], (&g_RageScratchpadState)->matrix.m[2][1],
                        (&g_RageScratchpadState)->matrix.m[2][2],
                        (&g_RageScratchpadState)->x0, (&g_RageScratchpadState)->y0,
                        (&g_RageScratchpadState)->x1, (&g_RageScratchpadState)->y1,
                        (&g_RageScratchpadState)->orderingFlag & 1,
                        ...
                        g_RageScratchpadState.envMode4, g_RandomSeed, g_AnimTimer,
