...
typedef struct RageCaptureModelDraw {
    ...
    uint8_t mirror;     /* g_RenderState.pass.orderingFlag at submission time */
    uint8_t table;      /* ordering table index the draw targets (0/1) */
    uint8_t fogged;
    uint8_t otShift;    /* g_RenderState.pass.otShift: one bucket is 4<<shift z units */
    uint8_t pad[3];
    int32_t modelIndex;
    int32_t otBaseBias; /* RENDER_OT_BASE offset from the table start */
    uint32_t renderMode; /* g_RenderState.geometry.envMode4 (CLUT row select) */
...
typedef struct RageSceneSnapshot {
    ...
    RageCaptureMatrix viewMatrix; /* (&g_RenderState.geometry.matrix) at frame end */
