...
/* Publish a source only after its initialization succeeds. Readiness and
 * provider selection are one state, not independently mutable flags. */
typedef enum ModernAssetSource {
    MODERN_ASSET_SOURCE_NONE,
    MODERN_ASSET_SOURCE_CACHE,
...
int ModernAssetsInit(void) {
    ...
    if (NativeAssetImporterReady()) {
        s_source = MODERN_ASSET_SOURCE_DISC;
        ++s_generation;
...
uint64_t ModernAssetsGeneration(void) { return s_generation; }
...
void ModernAssetsShutdown(void) {
    ...
    s_environmentIndexBytes = NULL;
    s_environmentIndexSize = 0;
    s_source = MODERN_ASSET_SOURCE_NONE;
    s_root[0] = '\0';
    s_modRoot[0] = '\0';
    s_modManifest = NULL;
    s_modReady = 0;
...
int ModernAssetsReady(void) {
    return (s_source != MODERN_ASSET_SOURCE_NONE);
}
...
int ModernAssetsLoadSkyImage(uint32_t assetKey,
    ...
    if (s_source == MODERN_ASSET_SOURCE_DISC)
        return NativeAssetImporterLoadSky(assetKey, &layout, image);
    if (s_source == MODERN_ASSET_SOURCE_NONE || bytes == NULL) return 0;
...
uint32_t ModernAssetsCachedMeshCount(void) {
    if (s_source == MODERN_ASSET_SOURCE_NONE) return 0;
    return (s_source == MODERN_ASSET_SOURCE_DISC) ? NativeAssetImporterMeshCount()
                            : s_cache.count;
}
