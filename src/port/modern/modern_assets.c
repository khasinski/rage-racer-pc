...
int ModernAssetsInit(void) {
    ...
    if (RageNativeAssetImporterReady()) {
        s_importerSource = 1;
        s_ready = 1;
...
void ModernAssetsShutdown(void) {
    ...
    RageNativeAssetImporterShutdown();
...
const RageRuntimeCachedMesh *ModernAssetsFind(
    ...
    if (s_importerSource) return RageNativeAssetImporterFind(instance);
...
int ModernAssetsLoadSkyImage(uint32_t assetKey, ModernAssetImage *image) {
    ...
    if (s_importerSource)
        return RageNativeAssetImporterLoadSky(assetKey, image);
...
uint32_t ModernAssetsCachedMeshCount(void) {
    if (!s_ready) return 0;
    return s_importerSource ? RageNativeAssetImporterMeshCount()
                            : s_cache.count;
}
...
static int ModernAssetsApplyModMaterialProperties(
    ...
    if (properties == NULL && RageAssetMaterialId(
        ...
        properties = RageModManifestFindMaterialProperties(
            &s_modManifest, baseId);
    return properties == NULL || RageRenderMaterialParseProperties(
...
int ModernAssetsLoadMaterial(const RageRenderMeshInstance *instance,
    ...
    if (s_importerSource) {
        ...
        if (!RageNativeAssetImporterLoadMaterial(
