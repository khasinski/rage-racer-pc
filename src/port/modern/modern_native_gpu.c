...
/*
 * The sky has a rotation of its own: the scene states where the backdrop
 * sits, because how it follows the camera is the game's rule rather than a
 * backend's choice. A frame from before it carried one keeps the camera's,
 * which is what such a frame was drawn with.
 */
static void ModernNativeBuildSkyCamera(const RageRenderCamera *camera,
                                       ModernNativeCameraUniform *out) {
    RageRenderCamera sky = *camera;
    const RageRenderQuaternion *rotation = &camera->skyOrientation;
    float length = sqrtf(rotation->x * rotation->x + rotation->y * rotation->y +
                         rotation->z * rotation->z + rotation->w * rotation->w);
    if (length > 0.0001f) {
        sky.transform.orientation = *rotation;
        sky.transform.hasOrientation = 1;
    }
    ModernNativeBuildCamera(&sky, out);
}
...
static void ModernNativeGpuDrawSet(
    ...
    {
        ModernNativeCameraUniform skyCamera;
        ModernNativeBuildSkyCamera(renderCamera, &skyCamera);
        skyCamera.projection[0] /= aspect;
        SDL_PushGPUVertexUniformData(command, 0, &skyCamera,
                                     sizeof(skyCamera));
    }
    ...
    /* The world is drawn from the real camera, not the sky's. */
    SDL_PushGPUVertexUniformData(command, 0, &camera, sizeof(camera));
