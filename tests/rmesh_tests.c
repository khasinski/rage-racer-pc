...
int main(void) {
    ...
    EXPECT(RageRuntimeMeshOpen(&mesh, blob, sizeof(blob)));
    EXPECT(RageRuntimeMeshRange(&mesh, 0, &first, &count));
    EXPECT(first == 0 && count == 6);
    EXPECT(RageRuntimeMeshVertex(&mesh, 0, &vertex));
    EXPECT((int)vertex.position[0] == 3 && vertex.color[2] == 3);
    EXPECT((int)(vertex.uv[0] * 100.0f) == 50 && vertex.material == 7);
    EXPECT(RageRuntimeMeshIndex(&mesh, 1, &index) && index == 2);
    blob[0] = 0;
    EXPECT(!RageRuntimeMeshOpen(&mesh, blob, sizeof(blob)));
    blob[0] = 'R';
    write_u32(blob + 28, 7);
