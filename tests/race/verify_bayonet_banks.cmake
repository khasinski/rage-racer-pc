...
foreach(asset RANGE 102 110 2)
    ...
    set(log "${EVIDENCE}/bayonet-${mode}-bank-${asset}.log")
    file(REMOVE "${log}")
    execute_process(COMMAND "${GAME}" --scenario "${SOURCE}/tests/scenarios/authored_bayonet.ini"
    ...
    if(NOT status EQUAL 0)
        message(FATAL_ERROR "Bayonet bank ${asset} race failed: ${status}\n${output}\n${errors}")
    endif()
    file(READ "${log}" trace)
    foreach(required "scenario-start rival=0 [^\n]*model=${model}"
            "authored Bayonet player body installed asset=46"
        ...
        if(NOT trace MATCHES "${required}")
            message(FATAL_ERROR "Bayonet bank ${asset} missed ${required}")
        endif()
    endforeach()
endforeach()
