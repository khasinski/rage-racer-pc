...
foreach(asset RANGE 128 134 2)
    ...
    set(log "${EVIDENCE}/vainqure-${mode}-bank-${asset}.log")
    file(REMOVE "${log}")
    execute_process(COMMAND "${GAME}" --scenario "${SOURCE}/tests/scenarios/authored_vainqure.ini"
    ...
    if(NOT status EQUAL 0)
        message(FATAL_ERROR "Vainqure bank ${asset} race failed: ${status}\n${output}\n${errors}")
    endif()
    file(READ "${log}" trace)
    ...
    foreach(required "scenario-start rival=0 [^\n]*model=${model}"
            "authored Vainqure player body installed asset=68"
            "authored Vainqure rival body installed asset=${asset}")
        if(NOT trace MATCHES "${required}")
            message(FATAL_ERROR "Vainqure bank ${asset} missed ${required}")
        endif()
    endforeach()
endforeach()
