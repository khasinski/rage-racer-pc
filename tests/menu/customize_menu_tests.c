...
int main(void) {
    g_CustomizeFadeAccum = 100;
    CHECK(DrawCustomizeScreen(0) == 0);

    CHECK(DrawCustomizeScreen(600) == MENU_FADE_MAX);
    CHECK(s_engineStep == 0);
    CHECK(s_enginePhase == MENU_FADE_MAX / 4);

    CHECK(DrawCustomizeScreen(-MENU_FADE_MAX) == 0);
    CHECK(s_engineStep == MENU_FADE_MAX * MENU_FADE_MAX / 2048);
    CHECK(s_enginePhase == 0);

    puts("customize menu tests passed");
    return 0;
}
