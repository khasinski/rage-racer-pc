...
(async()=>{
 ...
 const disc=process.env.RAGE_LAUNCHER_PROBE_CUE;let preparedRegion;
 ...
 try{
  if(disc){
   ...
   preparedRegion=service.state.disc.region;
  ...
  const work=async()=>{
   ...
   if(disc){
    ...
    const timing=preparedRegion==='PAL'?'timing=pal base_hz=50':'timing=ntsc base_hz=60';
    if(!log.includes('region='+preparedRegion)||!log.includes(timing)||log.includes('native asset cache '))throw Error('Packaged game used incorrect region/timing or a prebuilt cache');
